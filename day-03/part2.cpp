...
bool check_rect(_Field field, int mx, int my, int w, int h) {
...
int find_free_rect(_Field field, _Map claims) {
    for (_MapEntry& claim : claims) {
        auto margin = std::get<1>(claim);
        auto size = std::get<2>(claim);
        ...
        if (check_rect(field, mx, my, w, h)) {
            return std::get<0>(claim);
        }
    }
...
int main() {
    ...
    int id = find_free_rect(field, claims);
